...
#			V = -V0*(x-xc)**2
		...
		for n,xc_n in enumerate(xc) :
			V = V + Get_Potential_Operator(x,{'V0':V0,'xc':xc_n,'sigma':sigma},Pot_Type=6)
...
#	elif Pot_Type==9: # H2+
	...
	elif(Pot_Type==14): # Amonnia molecule
		V0		= InputData['V0']		if 'V0'		in InputData	else 1
...
def Get_Potential_Operator2D(x,y,InputData,Pot_Type=5):
	...
	if Pot_Type==5 or Pot_Type==24: #harmonic oscillator potential
		...
		# condition = abs(Y-ly/2) > ls/2 # slit 1
		# condition[abs(Y+ly/2) < ls/2] = False # slit 2
		# V[condition] = V0*exp(-abs(X[condition]/d*2))
