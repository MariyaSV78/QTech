...
def QM_calculations(InputData):
    ...
    if calctype == 1:
        ...
        # energy_f = -1/(2*(n_i - quant_def_f)**2);  # energy and ang momentum  # box parameter
        ...
        # energy_f = 0.3/nE * np.arange(1, nE + 1)
        ...
        for i_energy in range(nE):
            ...
            coul_f, coul_g = wf_Coulomb(l_partial_f, eta, z)

            # wave function should be energy-normalized    
            wf_normalization_factor = np.sqrt(2 / (np.pi * np.sqrt(2 * energy_f[i_energy])))
            chi_f[i_energy, :] = (coul_f * np.cos(np.pi * quant_def_i) + coul_g * np.sin(np.pi * quant_def_i)) * wf_normalization_factor

            r_fi[i_energy] = integrate(chi_f[i_energy, :] * r * chi_i, r)
            # r_fi[i_energy]=r_fi[i_energy]+chi_f[i_energy,ip]*r_coor[ip]* chi_i[ip]*dr
            r_fi[i_energy]=r_fi[i_energy]*wf_normalization_factor

        # # PI cross section
        # cross_section = np.abs(r_fi)**2/3 * 4 * np.pi**2 /137 * (energy_f - energy_i)
        ...
        # OutData.energy_f = energy_f * 27.3
        # OutData.cross_section = cross_section
        # OutData.cross_section2 = cross_section2
